(function() {
  ...
  .controller('grumblesController', function() {
    ...
    this.addComment = function(index) {
      ...
      // if no comments exist, create comments array
      if(!grumble.comments){
        grumble.comments = [];
      };
      
      grumble.comments.push(comment);
      this.resetComment();
    }
    this.deleteComment = function(grumbleIndex, commentIndex){
      var grumble = this.grumbles[grumbleIndex]
      var comment = grumble.comments[commentIndex]

      grumble.comments.splice(commentIndex, 1)
    }
    this.editComment = function(grumbleIndex, commentIndex){
      this.selectedComment = commentIndex; // for accessing comment in edit form
      var grumble = this.grumbles[grumbleIndex];
      var comment = grumble.comments[commentIndex]
      this.commentContent = comment.content
      this.commentAuthorName = comment.authorName
    }

    this.updateComment = function(index){
      var grumble = this.grumbles[index]
      var comment = grumble.comments[this.selectedComment]
      comment.authorName = this.commentAuthorName
    ...
    this.selectedComment = null; // temporarily store index value of comment being edited
