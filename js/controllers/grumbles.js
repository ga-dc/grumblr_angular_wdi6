(function() {
  ...
  // new comment controller (handles creation)
  commentControllers.controller('newCommentController', ["$location", 'Comment', function($location, Comments){
    this.create = function(){
      Comment.save(this.comment, function(grumbles, comments) {
        $location.path("/grumbles/" + grumbles.id + "/comments/" + comments.id);
      });
    };
  }]);
})();
